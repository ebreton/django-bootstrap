...
logger = logging.getLogger('myapp.user_activity')
...
def on_login(sender, **kwargs):
    ...
    logger.info("A User has logged in. Request : %s. User : %s" % (request, user))
...
def on_logged_out(sender, **kwargs):
    request = kwargs.get('request')
    user = kwargs.get('user')
    logger.info("A User has logged out. Request : %s. User : %s" % (request, user))
...
class LoggedModelAdminMixin(LogMixin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.is_dirty():
            ...
            super(LoggedModelAdminMixin, self).save_model(
                request=request,
                obj=obj,
                form=form,
                change=change)
            ...
            if modified_fields:
                self.logger.info(
                    "Django admin : changes in model %s,"
                    " fields : %s" % (self.__class__, ", ".join(modified_fields)))
